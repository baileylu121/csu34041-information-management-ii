...
export const quizQuestions: Question[] = [
  // ── Database Security & Access Control ──────────────────────────
  {
    ...
    question: 'A database administrator grants SELECT and UPDATE to user1, grants SELECT to user2, then revokes SELECT from user1. What happens to user1\'s privileges?',
    options: [
  ...
  },
  {
    id: 4,
    category: 'Concurrency Control',
    question: 'Under Strict Two-Phase Locking (Strict 2PL), T1 and T2 execute as follows: T1 acquires S-lock on X, T2 acquires S-lock on Y, T1 upgrades to X-lock on X, T2 upgrades to X-lock on Y, T1 requests lock on Y, T2 requests lock on X. What happens?',
    options: [
  ...
  },
  {
    id: 5,
    category: 'Concurrency Control',
    question: 'Wait-Die scheme: T1 (TS=1, oldest), T2 (TS=2), T3 (TS=3, youngest). T2 acquires X, T3 acquires Y, T1 requests X, T2 requests Y, T3 requests X. What happens?',
    options: [
      'T1 aborts',
      'T3 aborts',
      'T1 waits, T3 aborts',
  ...
  // ── SQL & Relational Algebra ────────────────────────────────────
  {
    id: 11,
    category: 'SQL',
    question: 'Given table Orders(order_id, customer_id, amount), which query returns customers whose total order amount exceeds 1000?',
    options: [
  ...
  },
  {
    id: 15,
    category: 'GDPR',
    question: 'An Irish e-commerce company obtains consent, later uses data for advertising (with updated consent), receives an erasure request, but deletes only from the main database while retaining data in backups and analytics datasets. What is the GDPR assessment?',
    options: [
  ...
  // ── Database Constraints ────────────────────────────────────────
  {
    ...
    question: 'Consider: ALTER TABLE Employees ADD CONSTRAINT chk_salary CHECK (salary > 0 AND salary < 100000). Which statement is TRUE?',
    options: [
  ...
  },
  {
    id: 18,
    category: 'Database Constraints',
    question: 'When a foreign key constraint is violated by a DELETE operation, which action propagates the change to referencing tuples?',
    options: [
      'SET NULL',
      'SET DEFAULT',
      'CASCADE',
  ...
  },
  {
    id: 20,
    category: 'ER Modelling',
    question: 'In the university database scenario, a student can enrol in many modules and a module can have many students. What cardinality constraint is this, and how is it mapped to relational schema?',
    options: [
  ...
  // ── Three-Level Architecture ────────────────────────────────────
  {
    ...
    question: 'In the three-level database architecture, which level describes "how data is physically stored" including indexes, compression, and encryption?',
    options: [
      'External (View) level',
  ...
  // ── ER to Relational Mapping ────────────────────────────────────
  {
    id: 28,
    category: 'ER Mapping',
    question: 'When mapping a 1:1 relationship from ER to relational schema, where should the foreign key be placed?',
    options: [
  ...
  },
  {
    id: 32,
    category: 'Database Security',
    question: 'What concurrency problem occurs when one transaction reads data written by another that then rolls back?',
    options: [
      'Lost update',
      'Dirty read',
      'Incorrect summary',
  ...
  },
  {
    id: 35,
    category: 'Concurrency Control',
    question: 'What concurrency problem occurs when one transaction calculates an aggregate while another updates the attributes being aggregated?',
    options: [
      'Lost update',
      'Dirty read',
      'Incorrect summary',
  ...
  // ── Additional Normalisation Questions ──────────────────────────
  {
    id: 38,
    category: 'Normalisation',
    question: 'What anomaly occurs when you cannot insert data without having other data (e.g., cannot add a new department without at least one employee)?',
    options: [
      'Deletion anomaly',
